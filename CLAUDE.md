# CLAUDE.md: Rishik Rontala's entry for Next Byte Hacks: V4
...
**Deadline: Wed Sep 30, 2026 · 11:45 PM EDT.** Event page: https://next-byte-hacks-v4.devpost.com/
...
- **GitHub Pages deploys automatically from whatever branch you push** (`.github/workflows/pages.yml` builds `dist/`, `build/`, `out/` or `site/`, or a root `index.html`). Live URL: **https://rishikrrontala-bot.github.io/next-byte-hacks-v4/**. After each deploy, check the Actions run succeeded and load the URL headlessly.
- Finish on `main` (judges read the default branch). Try `git push origin HEAD:main`. If that's refused, open a PR and merge it with the REST API (`gh api -X POST repos/rishikrrontala-bot/next-byte-hacks-v4/pulls -f title=… -f head=<branch> -f base=main`, then `gh api -X PUT repos/rishikrrontala-bot/next-byte-hacks-v4/pulls/<n>/merge -f merge_method=squash`). If that's refused too, make "merge the PR" step 1 of HANDOFF.md.
